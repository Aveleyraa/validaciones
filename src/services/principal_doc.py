    ...
    for i in t:
        ...
        if r[0]==1 and r[1]==0: #si es tabla general
            ...
            for i in inicios:
                ...
                if len(tuplas) == len(ntuplas):
                    ...
                    if fin == []:
                        hay = CommonUtils.variables(i,tuplas,freal,0,hoja,part_tab,ldeletras,'si')
                    else:
                        hay = CommonUtils.variables(i,tuplas,freal,fin[c]+pregunta,hoja,part_tab,ldeletras,'si') 
                    hay = [hay[0]]
                if c == len(fin):
                    break
                if len(tuplas) != len(ntuplas):
                    ...
                    for lista in tuplas:
                        ...
                        if lista[-1] == 'a':
                            lista.remove('a')
                            print('entro aqui 3')
                            hay = CommonUtils.validarTS(i,lista,freal,fin[c]+pregunta,hoja,ldeletras)
                        else:
                            print('entro aqui 4')
                            hay = CommonUtils.variables(i,lista,freal,fin[c]+pregunta,hoja,part_tab,ldeletras,'no') 
                            hay = hay[0]
                ...
                if fin[c]+pregunta == 0:
                    CommonUtils.menerrorsub(freal+2, a['final'][0]+2+pregunta, hoja, extraer_letras,r[9]+pregunta)
                if fin[c]+pregunta != 0:
                    CommonUtils.menerrorsub(freal+2, fin[c]+pregunta+2, hoja, extraer_letras,r[9]+pregunta)
                c+=1
        if r[1]!=0  and r[0]==1: #tablas de filas unicas
            ...
            for i in ad:
                ...
                if len(tuplas) == len(r[1]):
                    ...
                    hay = CommonUtils.variables(i,ntuplas,freal,0,hoja,part_tab,ldeletras,'si')
                    hay = hay[0]
                else:
                    extraer_letras=[]
                    part_tab = 0
                    ...
                    for lista in tuplas:
                        ...
                        if lista[-1] == 'a':
                            lista.remove('a')
                            hay = CommonUtils.validarTS(i,lista,freal,0,hoja,ldeletras)
                        
                        else:
                            hay = CommonUtils.variables(i,lista,freal,0,hoja,part_tab,ldeletras,'no')
                            hay = hay[0]
                        letras_validadas.append(hay)
                        extraer_letras.append(ldeletras[:6])
                        c+=1
                CommonUtils.menerrorsub(freal+2, freal+2, hoja, extraer_letras,r[9]+pregunta)
        ...
        if r[0]==2:
            ...
            for i in ad:
                ...
                if len(tuplas) == len(tp):
                    ldeletras= CommonUtils.letraslide(0)
                    hay = CommonUtils.vardelitos(i+1, p['tuplas'], freal,de,codelitos,
                    ...
                    for lista in tuplas:
                        ...
                        if lista[-1] == 'a':
                            lista.remove('a')
                            hay = CommonUtils.validarTSD(i,lista,freal,fin[k]+pregunta,hoja,ldeletras,pregunta+2,codelitos)
                            letras_validadas.append(hay)
                        else:
                            hay = CommonUtils.vardelitos(i+1, lista, freal,de,codelitos,
                               pregunta+2,fin[k]+pregunta+1,hoja,part_tab,ldeletras,'no')
                            letras_validadas.append(hay)
                        extraer_letras.append(ldeletras[:6])
                        c+=1
                
                CommonUtils.menerrorsub(freal+2, fin[k]+pregunta+2, hoja, extraer_letras,r[9]+pregunta)
                k+=1
        if r[2]==1 and r[0]==1: #tablas de unica columna de respuesta
            ...
            for i in ad:             
                freal = pregunta+i+1
                fin = r[6]
                CommonUtils.variablesl1(i, r[3], freal,fin[c]+pregunta,hoja,ldeletras)
                c+=1
